...
import { ChevronDown, ChevronRight, AlertTriangle, ExternalLink, Clock, Layers, Zap, TrendingUp, Flame, Gauge } from 'lucide-react';
...
export default function RaceCard({ race, runners, defaultExpanded = false }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Expand chevron */}
        <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-ink-100 transition-all ${expanded ? 'rotate-0' : ''} group-hover:bg-amber-100`}>
          {expanded ? (
        ...
        <div className="ml-2 flex-1 min-w-0">
          {race.probable_winner_name && (
            <div className="flex items-center gap-2">
              <span className="mono inline-flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-ink-900 text-xs font-bold text-ink-50 transition-transform hover:scale-110 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 pb-3">
        ...
        {race.confidence && (
          <span className={`badge sm:hidden ${confBadge.classes}`}>
            {confBadge.label}
          </span>
        )}
        <Link
        ...
        <div className="animate-slideDown border-t border-ink-100 px-4 py-4">
          ...
          {race.false_fav_flag && race.false_fav_reason && (
            <div className="mb-4 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 animate-fadeIn">
              <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg bg-amber-100">
